...
module.exports = function (options) {
	...
	// Render a given template with appropriate content.
	// Use cached pre-compiled template if possible, and populate cache for new templates.
	// 
	// @param templatePath [String] path to rendered template, used as key in cache
	// @param content [Object] object used to be rendered inside the template
	// @param callback [Function] end processing callback, invoked with arguments:
	// @option callback err [Error] an error object, or null if no error occured
	// @option callback html [String] Html rendering for this template
	function compileAndRender(templatePath, content, callback) {
	...
	function displayPage(path, res) {
		fs.exists(path, function(exists) {
			...
			if (pathUtils.extname(path).match(ext)) {
				...
				compileAndRender(path, values, function(err, content) {
					if (err) {
					...
					compileAndRender(pageTemplate, _.extend(values, {
						content: md(content),
				...
				res.contentType(path.substring(path.lastIndexOf(pathUtils.sep)+1));
	...
	app.post('/search', function(req, res){
		...
		search(searched, function(err, results) {
			...
			// in dev mode, clear cache immediately.
			if (!options.cache) {
				delete templates[searchTemplate];
			}
			// return http response
			compileAndRender(searchTemplate, {
				title: options.title,
	...
	makeSummary(function(err) {
		if (err) {
			return console.error('Failed to create pages summary:', err.message);
		}
